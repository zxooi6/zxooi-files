...
void moveServoUpAndDown() {
  ...
  unsigned long lastLightningSent = 0;
  int countTemp = 0;
  int countTimer = 0;
  ...
  while (true) {
    ...
    //Check from Lighting Sensor for strike 
    if (AS3935_ISR_Trig) {
    delay(5);
    AS3935_ISR_Trig = 0;
    ...
    //For MQTT control when Irr higher than preset value
    if(userManual == true && solarcell < IrrSet) {
    moveServoUp(false);
    isServoUp = true;
    }
    //For MQTT control when Irr lower than preset value
    if(userManual == true && solarcell > IrrSet) {
      lcd.clear();
      lcd.print(F("Servo down."));
      userManual = false;
    ...
    //Counter for resetting timer when low Irr
    if (countTimer >= 60) {                                                    
    ...
    //Act as a counter for timers and values
    if(currentMillis - lastIrradianceSent >= 1000) {
    countTemp++;
    countTimer++;
    lastIrradianceSent = currentMillis;
    }
    userManual = false;
  }
  delay(100);
